...
public class LegacyLandmarkPlacements {
    ...
    private static final Map<ResourceLocation, IafLandmark> BIOME_FEATURES = new ImmutableMap.Builder<ResourceLocation, IafLandmark>()
            //.put(BiomeKeys.DENSE_MUSHROOM_FOREST.location(), MUSHROOM_TOWER)
            .put(Biomes.PLAINS.location(), IafLandmark.FIRE_DRAGON_ROOST)
            .build();

    /**
     * @return The type of feature directly at the specified Chunk coordinates
     */
    public static IafLandmark getLandmarkDirectlyAt(int chunkX, int chunkZ, WorldGenLevel world) {
        if (blockIsInLandmarkCenter(chunkX << 4, chunkZ << 4)) {
            return pickLandmarkAtBlock(chunkX << 4, chunkZ << 4, world);
        }
        return IafLandmark.NOTHING;
    }
    ...
    public static IafLandmark pickLandmarkAtBlock(int blockX, int blockZ, WorldGenLevel world) {
     ...
     */
    public static IafLandmark pickLandmarkForChunk(int chunkX, int chunkZ, WorldGenLevel world) {
    ...
    public static IafLandmark pickBiomeLandmarkLegacy(RegistryAccess access, int chunkX, int chunkZ, Biome biome, long seed) {
        Optional<? extends Registry<Biome>> registryOpt = access.registry(Registry.BIOME_REGISTRY);

        if (registryOpt.isPresent()) {
            IafLandmark biomeFeature = BIOME_FEATURES.get(registryOpt.get().getKey(biome));
    ...
    public static IafLandmark pickVarietyLandmark(int chunkX, int chunkZ, long seed) {
        return null;
...
//            return IafLandmark.LICH_TOWER;
//        }
//
//        // also two nagas
//        if ((regionOffsetX == 5 && regionOffsetZ == 4) || (regionOffsetX == 3 && regionOffsetZ == 4)) {
//            return IafLandmark.NAGA_COURTYARD;
//        }
//
//        // get random value
//        // okay, well that takes care of most special cases
//        return switch (new Random(seed + chunkX * 25117L + chunkZ * 151121L).nextInt(16)) {
//            case 6, 7, 8 -> IafLandmark.MEDIUM_HILL;
//            case 9 -> IafLandmark.LARGE_HILL;
//            case 10, 11 -> IafLandmark.HEDGE_MAZE;
//            case 12, 13 -> IafLandmark.NAGA_COURTYARD;
//            case 14, 15 -> IafLandmark.LICH_TOWER;
//            default -> IafLandmark.SMALL_HILL;
//        };
    }
     ...
     */
    public static IafLandmark getNearestLandmark(int cx, int cz, WorldGenLevel world) {
     ...
     */
    public static IafLandmark getNearestLandmark(int cx, int cz, WorldGenLevel world, @Nullable Vec2i center) {
        int maxSize = IafLandmark.getMaxSearchSize();
        int diam = maxSize * 2 + 1;
        IafLandmark[] features = new IafLandmark[diam * diam];
        ...
        for (int rad = 1; rad <= maxSize; rad++) {
            for (int x = -rad; x <= rad; x++) {
                for (int z = -rad; z <= rad; z++) {
                    ...
                    IafLandmark directlyAt = features[idx];
                    if (directlyAt == null) {
     ...
     */
    public static IafLandmark getFeatureForRegion(int chunkX, int chunkZ, WorldGenLevel world) {
     ...
     * @return The feature in the chunk "region"
     */
    public static IafLandmark getFeatureForRegionPos(int posX, int posZ, WorldGenLevel world) {
    ...
    public static boolean isTheseFeatures(IafLandmark feature, IafLandmark... predicates) {
        for (IafLandmark predicate : predicates)
